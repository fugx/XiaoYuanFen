...
//        context.bindService(intent,musicServiceConn,BIND_AUTO_CREATE);
    ...
    public static VersionManager getInstance(Context context, String url) {
        ...
        if (ContextCompat.checkSelfPermission(context, Manifest.permission.SYSTEM_ALERT_WINDOW) != PackageManager.PERMISSION_GRANTED) {//申请WRITE_EXTERNAL_STORAGE权限
            ActivityCompat.requestPermissions((Activity)(context), new String[]{Manifest.permission.SYSTEM_ALERT_WINDOW}, 1);//自定义的code
        } else {
             intent = new Intent(context, ControlWindowService.class);
             intent.putExtra("url", url);
//            musicServiceConn = new MusicServiceConn();
